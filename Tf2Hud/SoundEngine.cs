...
public static class SoundEngine
{
    ...
    {
        if (path.IsNullOrEmpty() || !File.Exists(path)) PluginLog.Error($"Could not find file: {path}");
    ...
    public static void PlaySound(
        byte[] sound, bool useGameSfxVolume, int volume = 100, int sampleRate = 44100, int channels = 2,
        string? id = null)
    {
        var soundDevice = DirectSoundOut.DSDEVID_DefaultPlayback;
        new Thread(() =>
