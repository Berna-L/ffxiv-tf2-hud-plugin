...
public static class ColorComponent
{
        ...
        string id, SortedDictionary<ushort, ColorInfo> palette, ref ushort index, ushort defaultIndex)
    {
        ...
        foreach (var i in palette.Keys)
        {
            ...
            if (i % 15 != 0) ImGui.SameLine(0f, ImGui.GetStyle().ItemSpacing.Y);
        ...
        string tooltip = "")
    {
        ...
        if (ImGui.ColorButton($"##pickerButton{id}", palette[index].Vec4)) ImGui.OpenPopup(popupId);
        ...
        if (tooltip.Length > 0 && ImGui.IsItemHovered(ImGuiHoveredFlags.AllowWhenDisabled)) ImGui.SetTooltip(tooltip);
