        ...
        public object? Value
        {
            ...
            set
            {
                switch (Entry->Type)
                {
                    ...
                    {
                        if (!Entry->SetValueUInt(u32)) throw new Exception("Failed");
                        break;
                    }
                    case 3 when value is float f:
                    {
                        if (!Entry->SetValueFloat(f)) throw new Exception("Failed");
                        break;
                    }
                    case 4 when value is string s:
                    {
                        if (!Entry->SetValueString(s)) throw new Exception("Failed");
                        break;
                    }
                    default:
